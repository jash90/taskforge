...
const UNIT_MAP: Record<string, string> = {
  'km/h': 'km/h', 'm/s': 'm/s', 'km': 'km', 'm': 'm', 'cm': 'cm', 'mm': 'mm',
  'h': 'h', 'min': 'min', 's': 's', 'sek': 's', 'kg': 'kg', 'g': 'g',
  'l': 'l', 'ml': 'ml', '°C': '°C', 'K': 'K', 'N': 'N', 'J': 'J',
  'W': 'W', 'Pa': 'Pa', 'V': 'V', 'A': 'A', 'Ω': 'Ω',
  'zł': 'zł', 'PLN': 'zł', '$': '$', '€': '€', '%': '%',
};

const UNIT_ALIASES: Record<string, string> = {
  'kilometrow': 'km', 'kilometry': 'km', 'kilometr': 'km',
  'metrow': 'm', 'metry': 'm', 'metr': 'm',
  'gramow': 'g', 'gramy': 'g', 'gram': 'g',
  'kilogramow': 'kg', 'kilogramy': 'kg', 'kilogram': 'kg',
  'godzin': 'h', 'godziny': 'h', 'godzina': 'h',
  'minut': 'min', 'minuty': 'min', 'minuta': 'min',
  'sekund': 's', 'sekundy': 's', 'sekunda': 's',
  'stopni': '°C', 'stopnie': '°C',
  'procent': '%', 'procenty': '%',
};
...
export function detectParameters(text: string): TaskParameter[] {
  ...
  for (const pattern of patterns) {
    ...
    while ((match = pattern.exec(text)) !== null) {
      ...
      // Detect context: take ±20 chars before and after
      ...
      const min = isInteger ? Math.max(1, Math.floor(value * 0.3)) : Math.max(0.1, +(value * 0.3).toFixed(2));
  ...
  while ((sMatch = standalonePattern.exec(text)) !== null) {
    ...
    const isInteger = Number.isInteger(value);
    const min = isInteger ? Math.max(1, Math.floor(value * 0.2)) : Math.max(0.1, +(value * 0.2).toFixed(2));
...
export function applyParameters(content: string, parameters: TaskParameter[]): string {
  ...
  for (const p of sorted) {
    ...
    const regex = new RegExp(`(\\D|^)(${valStr.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})(\\s*${p.unit ? p.unit.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : ''})?(\\D|$)`, 'g');
...
export function renderParameterized(content: string, parameters: TaskParameter[]): string {
  ...
  for (const p of parameters) {
    const valStr = typeof p.value === 'number'
      ? (Number.isInteger(p.value) ? String(p.value) : p.value.toFixed(2).replace('.', ','))
      : String(p.value);
