...
import { isNestedCategoryFile, parseNestedCategories } from '@shared/services/db/categoryParser'
import { isMENCurriculum, parseMENCurriculum } from '@shared/services/db/menParser'
...
export function importCategoriesFromJSON(json: string): Category[] {
  ...
  if (!Array.isArray(cats)) {
    throw new Error(
      'Plik nie zawiera kategorii (oczekiwano formatu TaskForge lub zagnieżdżonego MEN).',
    )
  }
  for (const c of cats) {
...
export function importProgramPointsFromJSON(json: string): ProgramPoint[] {
  ...
  if (!Array.isArray(points)) {
    throw new Error(
      'Plik nie zawiera punktów podstawy programowej (oczekiwano formatu TaskForge lub MEN).',
    )
  }
...
type TextSegment = { kind: 'text'; value: string } | { kind: 'frac'; num: string; denom: string }

const FRAC_RE =
  /(?:(?<=^|[\s(])|^)(\d+(?:[.,]\d+)?\s*\/\s*\d+(?:[.,]\d+)?|[A-Za-zµΩ°][A-Za-zµΩ°²³¹]{0,3}\/[A-Za-zµΩ°][A-Za-zµΩ°²³¹]{0,3})(?=[\s.,;:!?)]|$)/g
...
export async function generateTestDocx(testTitle: string, tasks: Task[]): Promise<Blob> {
  ...
  children.push(
    new Paragraph({
      children: [
        new TextRun({
          text: 'Imię i nazwisko: ____________________________________________________',
          size: 22,
        }),
      ],
      spacing: { after: 200 },
  ...
  children.push(
    new Paragraph({
      children: [
        new TextRun({
          text: 'Data: ____________________      Klasa: ____________________',
          size: 22,
        }),
      ],
      spacing: { after: 360 },
    ...
    sections: [
      {
        ...
        children: [
          new Paragraph({
            ...
            alignment: AlignmentType.CENTER,
            children: [
              new TextRun({ text: `Klucz odpowiedzi – ${testTitle}`, bold: true, size: 32 }),
            ],
            spacing: { after: 240 },
          }),
          table,
          new Paragraph({
            ...
            children: [
              new TextRun({ text: `Razem punktów: ${totalPoints}`, bold: true, size: 22 }),
            ],
            spacing: { before: 240 },
          }),
        ],
