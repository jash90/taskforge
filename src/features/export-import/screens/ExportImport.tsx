...
import { downloadFile, exportTasksToJSON, importTasksFromJSON } from '../services/export'
  ...
  const performImport = async () => {
    ...
    try {
      ...
      for (const test of preview.tests) {
        await db.tests.put({
          ...test,
          id: test.id || `test-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
        })
      }
      ...
      if (preview.programPoints.length)
        parts.push(`${preview.programPoints.length} punktów podstawy`)
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (file && file.name.endsWith('.json')) {
      ...
      toast.error({
        ...
        description: 'Akceptujemy pliki .json wyeksportowane z TaskForge.',
      })
    }
  }
    ...
    <div className="grid-2">
      <div className="panel">
        ...
        <p className="text-muted text-sm mb-2 prose">
          Wyeksportuj wszystkie zadania i testy do pojedynczego pliku JSON. Możesz użyć go jako
          kopii zapasowej, albo przenieść dane na inne urządzenie.
        </p>
        <div className="card card-tight mb-2">
          ...
          <div className="text-sm">
            ...
            <span className="text-muted">
              {' '}
              · Testy: <strong>{tests?.length ?? 0}</strong>
            </span>
            <span className="text-muted">
              {' '}
              · Punkty podstawy: <strong>{programPoints?.length ?? 0}</strong>
            </span>
            <span className="text-muted">
              {' '}
              · Kategorie: <strong>{categories?.length ?? 0}</strong>
            </span>
          </div>
        </div>
      ...
      <div className="panel">
        ...
        <p className="text-muted text-sm mb-2 prose">
          Wczytaj wcześniej wyeksportowany plik JSON. Zadania i testy o tym samym ID zostaną
          nadpisane. Pokażemy podgląd przed zapisem.
        </p>
        <input
          ref={fileRef}
          type="file"
