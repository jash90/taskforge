...
import { buildTree, descendantIds, type CategoryNode } from '../utils/categoryTree'
...
export default function CategoryManager() {
  ...
  const saveEdit = async () => {
    ...
    if (!trimmed) {
      toast.error({ title: 'Nazwa nie może być pusta' })
      return
    }
  ...
  const handleExport = () => {
    ...
    downloadFile(
      json,
      `taskforge_kategorie_${new Date().toISOString().slice(0, 10)}.json`,
      'application/json',
    )
  ...
  const performImport = async () => {
    ...
    try {
      ...
      toast.success({
        title: 'Zaimportowano kategorie',
        description: `${importPreview.categories.length} kategorii`,
      })
    } catch (err) {
      toast.error({
  ...
  const performDelete = async () => {
    ...
    const affectedTasks = (tasks || []).filter((t) =>
      (t.categories || []).some((c) => idSet.has(c)),
    )
  ...
  const filteredTree = useMemo(() => {
    ...
    const matchNode = (n: CategoryNode): CategoryNode | null => {
      const filteredChildren = n.children
        .map(matchNode)
        .filter((x): x is CategoryNode => x !== null)
    ...
    <div>
      <div className="card mb-2">
        ...
        <div className="text-sm text-muted mt-1">
          ...
          {totalCategories === 1
            ? 'kategoria'
            : totalCategories % 10 >= 2 &&
                totalCategories % 10 <= 4 &&
                (totalCategories % 100 < 12 || totalCategories % 100 > 14)
              ? 'kategorie'
              : 'kategorii'}
          {' · '}
          {totalUsedTasks}{' '}
          {totalUsedTasks === 1
            ...
            : totalUsedTasks % 10 >= 2 &&
                totalUsedTasks % 10 <= 4 &&
                (totalUsedTasks % 100 < 12 || totalUsedTasks % 100 > 14)
              ? 'zadania przypisane'
              : 'zadań przypisanych'}
        </div>
      </div>
        ...
        <div className="card">
          {filteredTree.map((root) => (
            <CategoryTreeNode
              ...
              renderAfterRow={(n) =>
                addParentId === n.id ? (
                  <div className="subcategory-form" data-depth={Math.min(n.depth + 1, 8)}>
