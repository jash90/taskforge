...
import {
  downloadBlob,
  generateTestAnswerKeyDocx,
  generateTestDocx,
  taskPoints,
} from '@features/export-import'
import { randomizeParameter } from '@features/tasks'
...
import SavedTestsPanel from '../components/SavedTestsPanel'
import {
  CategoryFilterDialog,
  ProgramPointFilterDialog,
  TestPreviewDialog,
} from '../components/TestGeneratorDialogs'
...
export default function TestGenerator() {
  ...
  const [pageSize, setPageSize] = useState<number>(() => {
    const stored =
      typeof window !== 'undefined' ? window.localStorage.getItem('taskforge.tests.pageSize') : null
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((t) => {
      ...
      if (catFilterActive && !(t.categories || []).some((id) => expandedCategoryIds!.has(id)))
        return false
      if (!q) return true
      return (
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem('taskforge.tests.pageSize', String(pageSize))
    } catch {
      /* ignore */
    }
  }, [pageSize])
  ...
  const selectedTotalPoints = useMemo(() => {
    ...
    return tasks.filter((t) => ids.has(t.id)).reduce((sum, t) => sum + taskPoints(t), 0)
  }, [tasks, selectedTasks])
  ...
  const generate = async () => {
    ...
    setSelectedTasks([])
    toast.success({
      title: 'Utworzono test',
      description: `${test.tasks.length} zadań · ${selectedTotalPoints} pkt`,
    })
  }
    ...
    toast.success({
      ...
      action: {
        ...
        onPress: async () => {
          ...
          toast.info({ title: 'Przywrócono test' })
        },
      },
    })
  }
  ...
  const previewTasks = previewTest
    ? (tasks || [])
        .filter((t) => previewTest.tasks.includes(t.id))
        .map((t) => randomizedPreview.get(t.id) || t)
    : []
  ...
  return (
    <div>
        ...
        <div className="panel">
          ...
          <div className="form-group">
            ...
            <input
              ...
              placeholder="np. Sprawdzian z kinematyki"
            />
          </div>
          <div className="form-group">
          ...
          <div className="form-row">
            <label className="flex items-center gap-1 mb-0 label-plain">
              <input
                type="checkbox"
                className="icon-16"
                checked={settings.shuffleTasks}
                onChange={(e) => setSettings({ ...settings, shuffleTasks: e.target.checked })}
              />
            ...
            <div className="form-group mb-0">
              ...
              <input
                ...
                onChange={(e) =>
                  setSettings({ ...settings, timeLimitMinutes: parseInt(e.target.value) || 0 })
                }
              />
            </div>
          </div>
          ...
          <div className="flex justify-between items-center mt-2 mb-1 wrap gap-1">
            <strong className="text-sm">
              Wybrane: {selectedTasks.length}{' '}
              {selectedTotalPoints > 0 && (
                <span className="badge badge-success ml-px-6">{selectedTotalPoints} pkt</span>
              )}
            </strong>
          ...
          <div className="form-group mb-1">
            <div className="flex gap-1 wrap">
              ...
              >
                ...
                Podstawa programowa
                {filterPP.size > 0 && (
                  <span className="badge badge-on-primary">{filterPP.size}</span>
                )}
              </button>
              <button
                type="button"
              ...
              >
                ...
                {filterCategoryIds.length > 0 && (
                  <span className="badge badge-on-primary">{filterCategoryIds.length}</span>
                )}
              </button>
            </div>
                ...
                {filterCategoryIds.length > 0 && (
                  <>
                    <span
                      ...
                      data-spaced={filterPP.size > 0 || undefined}
                    >
                      Kategorie:
                    </span>
                  ...
                  <label key={t.id} className={`list-row ${checked ? 'selected' : ''}`}>
                    ...
                    <span className="col-tight">
                      ...
                      <span className="text-faint text-xs">
                        {t.subject} · {t.level} {t.class} · {points} pkt
                      </span>
                    </span>
                    <input
                      type="checkbox"
      ...
      <ConfirmDialog
        ...
        description={
          confirmDelete
            ? `„${confirmDelete.title}" zostanie usunięty. Tę akcję można cofnąć z paska powiadomień.`
            : ''
        }
        confirmLabel="Usuń"
        destructive
