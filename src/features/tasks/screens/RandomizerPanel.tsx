...
import { downloadFile } from '@features/export-import'
import {
  applyParameters,
  randomizeParameter,
  renderParameterized,
...
import type { RandomizedTask, Task, TaskChoice } from '@shared/types'
...
export default function RandomizerPanel({ task, onClose }: Props) {
  ...
  const generate = useCallback(() => {
    ...
    if (next.length > 0) {
      toast.info({
        title: `Wygenerowano ${next.length} ${next.length === 1 ? 'wariant' : next.length < 5 ? 'warianty' : 'wariantów'}`,
      })
    }
  }, [task, count])
  ...
  const exportAll = () => {
    ...
    if (exportFormat === 'json') {
      ...
      downloadFile(
        `${task.title}\n\n${lines}`,
        `${filenameBase}_warianty.txt`,
        'text/plain;charset=utf-8',
      )
    }
    toast.success({
      title: `Wyeksportowano ${instances.length} wariantów`,
