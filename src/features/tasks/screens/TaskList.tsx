...
import { CategoryPicker, buildTree, descendantIds, findNode, pathLabel } from '@features/categories'
import { copyAsWord, downloadFile, taskPoints } from '@features/export-import'
...
export default function TaskList({ onEdit, onRandomize, onNew }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return tasks.filter((t) => {
      ...
      const matchesCategories =
        !expandedCategoryIds || (t.categories || []).some((id) => expandedCategoryIds.has(id))
  ...
  const filterCount =
    (filterSubject ? 1 : 0) + (filterLevel ? 1 : 0) + (filterCategoryIds.length > 0 ? 1 : 0)
  ...
  return (
    <div>
      ...
      <div className="card mb-2">
        <div className="flex gap-1 wrap items-center">
          ...
          >
            ...
            {filterCount > 0 && <span className="badge badge-primary">{filterCount}</span>}
          </button>
          {(filterCount > 0 || query) && (
      ...
      {selected.size > 0 && (
        <div className="card card-row mb-2">
          <span className="text-sm font-semibold">
            ...
            {selected.size === 1 ? 'zadanie' : selected.size < 5 ? 'zadania' : 'zadań'}
          </span>
        ...
        <>
          <div className="flex items-center gap-1 mb-1 pl-px-6">
            ...
            <span className="text-faint text-xs">
              {filtered.length}{' '}
              {filtered.length === 1 ? 'zadanie' : filtered.length < 5 ? 'zadania' : 'zadań'}
            </span>
          </div>
                      ...
                      <div className="flex items-center gap-1 wrap">
                        ...
                        {task.aiGenerated && (
                          <span
                            ...
                            title={
                              task.aiModel
                                ? `Wygenerowane przez ${task.aiModel}`
                        ...
                        <span className="badge badge-info">
                          {task.level} · {task.class}
                        </span>
                        {totalPoints > 0 && (
                          <span className="badge badge-success">{totalPoints} pkt</span>
                        )}
                      </div>
                      ...
                      <div className="task-meta">
                        ...
                        {task.programPoints.length > 0 && (
                          <span className="text-sm text-faint">
                            {task.programPoints.length} pkt podstawy
                          </span>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="task-actions">
                    ...
                    <OverflowMenu
                      ...
                      items={[
                        {
                          id: 'rand',
                          label: 'Losuj warianty',
                          icon: <Shuffle size={14} aria-hidden="true" />,
                          onSelect: () => onRandomize(task),
                        },
                        {
                          id: 'word',
                          label: 'Eksportuj jako Word',
                          icon: <FileSpreadsheet size={14} aria-hidden="true" />,
                          onSelect: () => exportTaskWord(task),
                        },
                        {
                          id: 'del',
                          label: 'Usuń zadanie',
                          icon: <Trash2 size={14} aria-hidden="true" />,
                          onSelect: () => setConfirmOne(task),
                          variant: 'danger',
                          divider: true,
                        },
                      ]}
      ...
      {categoryPickerOpen && (
        <div
          className="overlay"
          onMouseDown={() => setCategoryPickerOpen(false)}
          role="presentation"
        >
          <div
          ...
          >
            <div className="flex justify-between items-center mb-2">
              <h2 id="cat-filter-title" className="card-title mb-0">
      ...
      <ConfirmDialog
        ...
        description={
          confirmOne
            ? `„${confirmOne.title}" zostanie trwale usunięte z bazy. Tę akcję można cofnąć z paska powiadomień.`
            : ''
        }
        confirmLabel="Usuń"
        destructive
