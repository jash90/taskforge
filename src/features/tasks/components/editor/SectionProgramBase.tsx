...
export default function SectionProgramBase({
  ...
  const hasMatchesOutsideScope = useMemo(() => {
    ...
    const q = query.toLowerCase()
    const inScopeMatches = inScope.filter(
      (p) =>
        p.code.toLowerCase().includes(q) ||
        p.description.toLowerCase().includes(q) ||
        (p.dzialNazwa || '').toLowerCase().includes(q),
  ...
  const grouped = useMemo(() => {
    ...
    for (const p of list) {
      const groupKey = allLevels
        ...
        : p.dzialNumer
          ? `${p.dzialNumer}. ${p.dzialNazwa || 'Inne'}`
          : 'Inne'
      ;(acc[groupKey] ??= []).push(p)
    }
    ...
    <div>
      ...
      {selectedCount > 0 && (
        <div className="card card-tight card-accent mt-1 mb-1">
          <div className="flex justify-between items-center mb-1">
            <strong className="text-sm color-accent">Wybrane punkty ({selectedCount})</strong>
            <button
              type="button"
      ...
      {hasMatchesOutsideScope && (
        <div className="text-sm info-banner mb-1" role="status">
          Brak wyników w bieżącej klasie. Punkty pasujące do „{query}" istnieją w innych klasach lub
          poziomach.{' '}
          <button type="button" className="btn-link-inline" onClick={() => setAllLevels(true)}>
            Pokaż wszystkie
          </button>
        </div>
      )}
