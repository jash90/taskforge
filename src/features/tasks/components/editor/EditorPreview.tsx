...
import type { TaskChoice, TaskParameter } from '@shared/types'
...
export default function EditorPreview({
  ...
  const renderChoice = (c: TaskChoice) =>
    mode === 'with' ? renderParameterized(c.content, parameters) : c.content
            ...
            <ol className="choices-list preview-list is-tight">
              {choices.map((c, i) => (
                <li key={c.id}>
                  ...
                  {renderChoice(c) || <em className="em-placeholder">—</em>}
                </li>
              ))}
            </ol>
          )}
      ...
      <div className="flex gap-1 wrap">
        ...
        >
          {copied ? (
            ...
            <>
              <Copy size={14} aria-hidden="true" /> Kopiuj jako Word
            </>
          )}
        </button>
        <button
