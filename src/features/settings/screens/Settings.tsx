...
import { fetchKeyInfo } from '@features/ai-generation'
...
export default function Settings() {
  ...
  const [promptDraft, setPromptDraft] = useState<string>(
    settings.aiSystemPrompt || DEFAULT_SYSTEM_PROMPT,
  )
    ...
    <div className="max-w-920">
      ...
      <section className="card mb-2">
        ...
        <p className="text-muted text-sm mb-2 prose">
          OpenRouter to bramka do setek modeli LLM (GPT, Claude, Gemini, Llama, DeepSeek, …) z
          jednym kluczem API. Klucz przechowywany jest <strong>wyłącznie lokalnie</strong> w
          przeglądarce (localStorage) i wysyłany tylko bezpośrednio do openrouter.ai.{' '}
          <a href="https://openrouter.ai/keys" target="_blank" rel="noreferrer">
            Pobierz klucz{' '}
            <ExternalLink size={11} aria-hidden="true" className="icon-external-inline" />
          </a>
        </p>
        ...
        {keyInfo && (
          <div className="card card-tight card-success mt-1">
            ...
            <div className="text-sm mt-px-4">
              {keyInfo.label && (
                <>
                  Etykieta: <strong>{keyInfo.label}</strong> ·{' '}
                </>
              )}
              {typeof keyInfo.limit === 'number' ? (
                <>
                  Limit: ${keyInfo.limit.toFixed(2)} · Zużyto: ${(keyInfo.usage ?? 0).toFixed(4)} ·
                  Pozostało: <strong>${(keyInfo.limit - (keyInfo.usage ?? 0)).toFixed(2)}</strong>
                </>
              ) : (
      ...
      <section className="card mb-2">
        ...
        <p className="text-muted text-sm mb-2 prose">
          Wybierz model używany do generowania zadań. Lista jest pobierana z OpenRouter i zawiera
          ceny oraz długość kontekstu. Polecane modele (oznaczone gwiazdką) sprawdzają się dobrze do
          tworzenia treści po polsku.
        </p>
      ...
      <section className="card">
        ...
        <p className="text-muted text-sm mb-1">
          Usuwa zapisany klucz API, model i prompt z tego urządzenia. Nie wpływa na zadania ani
          kategorie.
        </p>
        <button
          type="button"
      ...
      <p className="text-xs text-faint mt-2">
        ...
        Zadania wygenerowane przez AI są oznaczane w bazie zadań — możesz je odróżnić od ręcznie
        utworzonych.
      </p>
    </div>
  )
