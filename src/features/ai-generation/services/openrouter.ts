...
const polishError = (status: number, body?: { error?: { message?: string } }): string => {
  ...
  if (status >= 500)
    return apiMsg ? `Błąd serwera OpenRouter: ${apiMsg}` : 'Błąd po stronie OpenRouter.'
...
export async function generateChat(opts: GenerateOptions): Promise<GenerationResult> {
  ...
  const onParentAbort = () =>
    ctrl.abort((opts.signal as AbortSignal & { reason?: unknown })?.reason)
  if (opts.signal) {
  ...
  try {
    ...
    if ((err as Error)?.name === 'AbortError' || (err as Error)?.name === 'TimeoutError') {
      ...
      if (
        ctrl.signal.reason instanceof DOMException &&
        ctrl.signal.reason.name === 'TimeoutError'
      ) {
        throw new Error(
          `Model nie odpowiedział w ciągu ${Math.round((opts.timeoutMs ?? 90_000) / 1000)} s.`,
          { cause: err },
        )
      }
    }
