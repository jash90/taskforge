...
import ProgramBaseConfirmDialogs from '../components/ProgramBaseConfirmDialogs'

type DrawerMode = { kind: 'closed' } | { kind: 'add' } | { kind: 'edit'; point: ProgramPoint }
...
export default function ProgramBaseViewer() {
  ...
  const [pageSize, setPageSize] = useState<number>(() => {
    const stored =
      typeof window !== 'undefined' ? window.localStorage.getItem('taskforge.pp.pageSize') : null
  ...
  const subjects = useMemo(
    () => Array.from(new Set((points || []).map((p) => p.subject))).sort(),
    [points],
  )
  const classes = useMemo(
    () => Array.from(new Set((points || []).map((p) => p.class))).sort(),
    [points],
  )
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem('taskforge.pp.pageSize', String(pageSize))
    } catch {
      /* ignore */
    }
  }, [pageSize])
  ...
  const deletePoint = async (pp: ProgramPoint) => {
    ...
    toast.success({
      ...
      action: {
        label: 'Cofnij',
        onPress: async () => {
          await db.programPoints.put(pp)
          toast.info({ title: 'Przywrócono punkt' })
        },
      },
    })
  }
  ...
  const handleExport = () => {
    ...
    downloadFile(
      json,
      `taskforge_podstawa_${subjectSlug}_${new Date().toISOString().slice(0, 10)}.json`,
      'application/json',
    )
  ...
  const performImport = async () => {
    ...
    try {
      ...
      toast.success({
        title: 'Zaimportowano podstawę',
        description: `${importPreview.points.length} punktów`,
      })
    } catch (err) {
      toast.error({
  ...
  return (
    <div>
      ...
      <div className="card mb-2">
        <div className="form-row mb-0">
          <div className="form-group mb-0 grow">
            <label htmlFor="pp-search-main" className="sr-only">
              Szukaj
            </label>
          ...
          <div className="form-group mb-0 flex-200">
            <label htmlFor="pp-subject" className="sr-only">
              Przedmiot
            </label>
          ...
          <div className="form-group mb-0 flex-200">
            <label htmlFor="pp-level" className="sr-only">
              Poziom
            </label>
            <select
              id="pp-level"
              value={level}
              onChange={(e) => setLevel(e.target.value as ProgramPoint['level'] | '')}
            >
          ...
          <div className="form-group mb-0 flex-160">
            <label htmlFor="pp-class-filter" className="sr-only">
              Klasa
            </label>
          ...
          <div className="form-group mb-0 flex-160">
            <label htmlFor="pp-zakres" className="sr-only">
              Zakres
            </label>
            <select
              id="pp-zakres"
              value={zakres}
              onChange={(e) => setZakres(e.target.value as '' | 'podstawowy' | 'rozszerzony')}
            >
        ...
        <div className="flex justify-between items-center mt-1 wrap gap-1">
          <span className="text-sm text-muted">
            {filtered === undefined ? (
              ...
              <>
                Znaleziono <strong>{filtered.length}</strong> punktów
              </>
            )}
          </span>
        ...
        grouped.map(([dzial, pts]) => (
          <div key={dzial} className="card mb-1">
            <div className="card-title card-title-md">
              {dzial}
              <small>
                {pts.length} {pts.length === 1 ? 'punkt' : pts.length < 5 ? 'punkty' : 'punktów'}
              </small>
            </div>
      ...
      {drawer.kind !== 'closed' && (
        <>
          ...
          <aside
            ...
            role="dialog"
            aria-modal="true"
            aria-label={drawer.kind === 'edit' ? 'Edytuj punkt podstawy' : 'Dodaj punkt podstawy'}
          >
            ...
            <div className="drawer-body">
              ...
              <div className="form-row">
                <div className="form-group mb-0">
                  ...
                  <select
                    ...
                    value={form.level}
                    onChange={(e) =>
                      setForm({ ...form, level: e.target.value as ProgramPoint['level'] })
                    }
                  >
                    <option value="podstawowa">Szkoła podstawowa</option>
                    <option value="ponadpodstawowa">Liceum / technikum</option>
              ...
              <div className="form-row">
                <div className="form-group mb-0">
                  ...
                  <input
                    id="np-dzial-nr"
                    value={form.dzialNumer}
                    onChange={(e) => setForm({ ...form, dzialNumer: e.target.value })}
                    placeholder="np. II"
                  />
                </div>
                <div className="form-group mb-0">
                  ...
                  <input
                    id="np-dzial-nazwa"
                    value={form.dzialNazwa}
                    onChange={(e) => setForm({ ...form, dzialNazwa: e.target.value })}
                    placeholder="np. Ruch i siły"
                  />
                </div>
              </div>
              <div className="form-group mb-0">
            ...
            <div className="drawer-footer">
              ...
              <button type="button" className="btn btn-primary" onClick={savePoint}>
                ...
                {drawer.kind === 'edit' ? 'Zapisz zmiany' : 'Zapisz punkt'}
              </button>
            </div>
          </aside>
        </>
      ...
      <ProgramBaseConfirmDialogs
        confirmDelete={confirmDelete}
        importPreview={importPreview}
        onConfirmDelete={() => {
          if (confirmDelete) {
            void deletePoint(confirmDelete)
