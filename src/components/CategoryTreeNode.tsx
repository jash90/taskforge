...
export default function CategoryTreeNode({
  node, expanded, onToggleExpand,
  renderLeading, renderTrailing, rowClassName, onSelect,
  indent = 18,
  renderAfterRow,
}: Props) {
    ...
    <>
      ...
      >
        <button
          ...
          aria-label={hasChildren ? (isExpanded ? `Zwiń ${node.name}` : `Rozwiń ${node.name}`) : undefined}
