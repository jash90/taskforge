...
import {
  Database, Pencil, Shuffle, FileText, GraduationCap, Upload,
  Plus, Sun, Download, BookOpen, FolderTree, Sparkles,
...
interface Props {
  open: boolean;
  onClose: () => void;
  onNavigate: (tab: 'tasks' | 'editor' | 'randomizer' | 'tests' | 'program' | 'categories' | 'export' | 'ai' | 'settings') => void;
  onNewTask: () => void;
...
export default function CommandPalette({
  open, onClose, onNavigate, onNewTask, onEditTask, onExportJson, onCycleTheme,
}: Props) {
  const recentTasks: Task[] | undefined = useLiveQuery(
    async () => {
  ...
  const allCommands: PaletteCommand[] = useMemo(() => {
    const nav: PaletteCommand[] = [
      { id: 'nav-tasks',   group: 'Nawigacja', label: 'Przejdź do bazy zadań',          hint: 'g t', icon: <Database size={16} />,      run: () => onNavigate('tasks') },
      { id: 'nav-editor',  group: 'Nawigacja', label: 'Przejdź do edytora',             hint: 'g e', icon: <Pencil size={16} />,         run: () => onNavigate('editor') },
      { id: 'nav-rand',    group: 'Nawigacja', label: 'Przejdź do losowania',           hint: 'g r', icon: <Shuffle size={16} />,        run: () => onNavigate('randomizer') },
      { id: 'nav-tests',   group: 'Nawigacja', label: 'Przejdź do testów',              hint: 'g s', icon: <FileText size={16} />,       run: () => onNavigate('tests') },
      { id: 'nav-program', group: 'Nawigacja', label: 'Przejdź do podstawy programowej',hint: 'g p', icon: <GraduationCap size={16} />, run: () => onNavigate('program') },
    ...
    const acts: PaletteCommand[] = [
      { id: 'act-new',    group: 'Akcje', label: 'Nowe zadanie',                  hint: '⌘N', icon: <Plus size={16} />,     run: onNewTask },
      { id: 'act-export', group: 'Akcje', label: 'Eksportuj bazę do JSON',         icon: <Download size={16} />, run: onExportJson },
      { id: 'act-theme',  group: 'Akcje', label: 'Przełącz motyw (jasny / ciemny / system)', icon: <Sun size={16} />, run: onCycleTheme },
    ];
    ...
    <div className="palette-backdrop" onMouseDown={onClose} role="presentation">
      ...
      >
        ...
        {filtered.length === 0 ? (
          ...
          <ul ref={listRef} className="palette-list" role="listbox">
            {filtered.map((c) => {
              ...
              return (
                <div key={c.id}>
                  {isNewGroup && <li className="palette-section" role="presentation">{c.group}</li>}
                  <li
                    role="option"
        ...
        <div className="palette-footer">
          ...
          <span><kbd>Enter</kbd> wybierz · <kbd>Esc</kbd> zamknij</span>
        </div>
      </div>
    </div>
  );
